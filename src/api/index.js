...
const {
  verificaLogin, rotaUsuarioLogado, rotaUsuarioNaoLogado, rotaSuperUsuarioLogado,
} = require('../auth');
const User = require('../models/User');
const db = require('../db');
const { getSalasDisponiveis } = require('../utils');
  ...
  try {
    ...
    if (senha && novaSenha) {
      ...
      if (user.senha === senhaHash) {
        user.senha = crypto.createHash('sha256').update(novaSenha).digest('hex');
        alterou = true;
      } else {
...
router.post('/alterarSkin', rotaSuperUsuarioLogado, async (req, res) => {
  const {
    id, nome, imagem, tipoPeca, corTematica,
  } = req.body;
  ...
  try {
    const result = await db.query(query, {
      replacements: {
        nomeSkin: nome, imagemSkin: imagem, tipoPeca, corTematica, id,
      },
...
router.post('/alterarTabuleiro', rotaSuperUsuarioLogado, async (req, res) => {
  ...
  try {
    const result = await db.query(query, {
      replacements: {
        nomeTabuleiro: nome, imagemTabuleiro: imagem, corTematica, id,
      },
      type: QueryTypes.UPDATE,
    });
    res.json(result);
...
router.post('/alterarSeason', rotaSuperUsuarioLogado, async (req, res) => {
  const {
    id, nome, inicioVigencia, fimVigencia, idTabuleiro, prioridade,
  } = req.body;
...
router.post('/alterarVenda', rotaSuperUsuarioLogado, async (req, res) => {
  const {
    id, valor, idSeason, idSkin,
  } = req.body;
  if (!parseInt(valor, 10) || valor < 1) {
...
router.post('/inserirSkin', rotaSuperUsuarioLogado, async (req, res) => {
  const {
    nome, imagem, tipoPeca, corTematica,
  } = req.body;
  ...
  try {
    const result = await db.query(query, {
      replacements: {
        nomeSkin: nome, imagemSkin: imagem, tipoPeca, corTematica,
      },
...
router.post('/inserirTabuleiro', rotaSuperUsuarioLogado, async (req, res) => {
  ...
  try {
    const result = await db.query(query, {
      replacements: { nomeTabuleiro: nome, imagemTabuleiro: imagem, corTematica },
      type: QueryTypes.INSERT,
    });
    res.json(result);
  } catch (error) {
...
router.post('/inserirSeason', rotaSuperUsuarioLogado, async (req, res) => {
  const {
    nome, inicioVigencia, fimVigencia, idTabuleiro, prioridade,
  } = req.body;
...
router.post('/inserirVenda', rotaSuperUsuarioLogado, async (req, res) => {
  ...
  try {
    const result = await db.query(query, {
      replacements: { valor, idSeason, idSkin },
      type: QueryTypes.INSERT,
    });
    res.json(result);
  } catch (error) {
...
router.post('/isPeriodoDisponivelSeason', rotaSuperUsuarioLogado, async (req, res) => {
  ...
  try {
    const result = await db.query(query, {
      replacements: {
        inicioVigencia: inicio, fimVigencia: fim, prioridade, id,
      },
      type: QueryTypes.SELECT,
    });
    console.log(result);
