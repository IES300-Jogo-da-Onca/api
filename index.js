...
app.use(
  session({
    ...
    name: KEY,
    cookie: {
      sameSite: 'none', // Permite o envio do cookie entre o domínio do front e do back
      secure: true, // Exige HTTPS (obrigatório ao usar sameSite 'none')
    },
  }),
)
...
db.authenticate()
  .then(() =>
    server.listen(process.env.PORT || 3001, () =>
      console.log(process.env.HOST || 'http://localhost:3001'),
    ),
  )
  .catch(console.error)
...
io.on('connection', (socket) => {
  console.log(
    `user ${socket.handshake.session.user.nome} conectou. socket id: ${socket.id}`,
  )
  socket.on('novaSala', (pecaSelecionada) => {
    ...
    if (salaAntiga) {
      ...
      console.log(
        socket.handshake.session.user.nome + ' deixou a sala ' + salaAntiga,
      )
    }
    socket.join(novaSala)
    console.log(
      socket.handshake.session.user.nome + ' entrou na sala ' + novaSala,
    )
  ...
  socket.on('moverPeca', (data) => {
    ...
    if (
      ...
      !Jogo.ehMovimentoValido(
        ...
        tabuleiro,
        ehCachorro,
        sala.dadosPartida.houveCaptura,
      )
    ) {
    ...
    novoTabuleiro = Jogo.getNovoTabuleiro(
      tabuleiro,
      x,
      y,
      old_x,
    ...
    if (houveCaptura) {
      ...
      oncaContinuaCaptura = Jogo.getPossiveisMovimentos(
        x,
        y,
        false,
        novoTabuleiro,
...
const emitirFimDeJogo = (sala, oncaVenceu) => {
  ...
  const nomeVencedor = oncaVenceu
    ? sala.nomeJogadorOnca
    : sala.nomeJogadorCachorro
